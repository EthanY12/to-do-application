...
describe('Login Component Unit Tests', () => {
  ...
  test('calls login service on form submit', async () => {
    authService.login.mockResolvedValue({ user: { id: 1, username: 'testuser', token: 'testtoken' } });
    ...
    expect(localStorage.setItem).toHaveBeenCalledWith('user', JSON.stringify({ id: 1, username: 'testuser', token: 'testtoken' }));
  });
